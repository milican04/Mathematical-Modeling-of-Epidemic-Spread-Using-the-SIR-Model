...
To rigorously assess the performance of the RK4 solver on this localized epidemic wave, the fit is quantified using a combination of standard statistical error metrics and domain-specific epidemiological checks based on the data presented in Figure \ref{fig:single_wave}:

\begin{itemize}
    \item \textbf{Root Mean Squared Error (RMSE):} Measures the standard deviation of the residuals. Because RMSE squares the errors before averaging them, it heavily penalizes large misses, making it ideal for highlighting how a model fits the height of an epidemic peak. \textbf{RMSE: 504.86}
    \item \textbf{Mean Absolute Error (MAE):} Measures the average absolute distance between the observed data and the predicted curve. MAE treats all errors equally, making it less sensitive to isolated reporting outliers and providing a clean view of the model's average day-to-day deviation. \textbf{MAE: 390.12}
    \item \textbf{Peak Timing and Magnitude Error:} Evaluates the model's practical forecasting ability. Peak Timing Error measures the difference in days between the real and predicted peaks, while Peak Magnitude Error measures the difference in the maximum height of the wave. \textbf{Peak Timing Error: 5 days} $\mid$ \textbf{Peak Magnitude Err: 850 cases (15.5\%)}
    \item \textbf{Coefficient of Determination ($R^2$):} Represents the proportion of variance in the dependent variable that is mathematically predictable from the model. \textbf{$R^2$: 0.8459}
    \item \textbf{Akaike and Bayesian Information Criteria (AIC/BIC):} Evaluate goodness-of-fit while penalizing model complexity (number of parameters). These ensure the underlying mathematics are genuinely improved rather than simply overfitted to data noise. \textbf{AIC: 1236.41 $\mid$ BIC: 1241.60}
\end{itemize}
...
During the preparation of this project, the author utilized the Gemini artificial intelligence model to assist with structuring the document and translating text. The author reviewed and refined all AI-generated output and takes full responsibility for the final content and accuracy of this work.

\begin{thebibliography}{9}

\bibitem{brauer2008}
Brauer, F., van den Driessche, P., \& Wu, J. (Eds.). (2008). 
\textit{Mathematical Epidemiology (Lecture Notes in Mathematics)}. 
Springer.

\bibitem{mubarez}
Mubarez, G. S., \& Mohsini, M. N. 
\textit{SIR Model and its Applications}. 
Research Article.
